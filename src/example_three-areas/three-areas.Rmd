...
Spatial smoothing is then described as "borrowing information between areas", despite the fact that the information is a property of individuals, and not of space.
...
# For some reason this is crashing INLA now, need to debug, but it worked before..
...
experiment_A <- function(yA, mA) {
  ...
  return(list("mean" = fit$summary.fitted.values[2, "mean"], "mode" = fit$summary.fitted.values[2, "mode"]))
}
